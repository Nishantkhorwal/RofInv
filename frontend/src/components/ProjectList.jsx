...
const ProjectList = () => {
  ...
  const [isPanCardImageFilled, setIsPanCardImageFilled] = useState(false);
  ...
  const handleFormChange = (e) => {
    ...
    if (type === "file" && files.length > 0) {
      ...
      if (name === "chequeImage") {
        ...
        setIsPanCardImageFilled(true); // Update PAN card filled state
      }
    } else {
      setFormData({
        ...formData,
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Sidebar */}
      <div
        className={`bg-[#F5F5DC] z-50 fixed lg:relative h-full transition-all duration-300 ${isSidebarOpen ? 'w-64 left-0' : '-left-64 w-0'
          } flex-shrink-0 top-0`}
      >
        ...
        <ul className="mt-4 px-4 space-y-2">
          ...
          <li
            ...
            className={`text-gray-600 hover:bg-gray-300 px-2 py-2 rounded cursor-pointer ${activeTab === 'projects' ? 'bg-gray-300' : ''
              }`}
          >
            Projects
          </li>
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const fileInput = document.getElementById("panCapture");
                      if (fileInput) {
                ...
                {/* Hidden Camera Capture Input for PAN Card */}
                <input
                  type="file"
                  id="panCapture"
                  name="panCardImage"
                ...
                {/* Display Selected PAN Card File Name */}
                {fileNames.panCardImage && (
                  <p className="mt-2 text-sm text-gray-600">
                    Selected File: <span className="font-bold">{fileNames.panCardImage}</span>
                  </p>
                )}
              </div>
